...
data=( `cat /usr/local/etc/xray/config.json | grep '#vl' | cut -d ' ' -f 2 | sort | uniq`);
echo "-------------------------------";
...
function renewws(){
clear
NUMBER_OF_CLIENTS=$(grep -c -E "^#vl " "/usr/local/etc/xray/config.json")
    ...
    echo -e "\033[1;93m─────────────────────────────────────\033[0m"
    echo ""
  	grep -E "^#vl " "/usr/local/etc/xray/config.json" | cut -d ' ' -f 2-3 | column -t | sort | uniq
    echo ""
    ...
    exp=$(grep -wE "^#vl $user" "/usr/local/etc/xray/config.json" | cut -d ' ' -f 3 | sort | uniq)
    now=$(date +%Y-%m-%d)
    d1=$(date -d "$exp" +%s)
    d2=$(date -d "$now" +%s)
    exp2=$(( (d1 - d2) / 86400 ))
    ...
    sed -i "/#vl $user/c\#vl $user $exp4" /usr/local/etc/xray/config.json
...
NUMBER_OF_CLIENTS=$(grep -c -E "^#vl " "/usr/local/etc/xray/config.json")
    ...
    exp=$(grep -wE "^#vl $user" "/usr/local/etc/xray/config.json" | cut -d ' ' -f 3 | sort | uniq)
    sed -i "/^#vl $user $exp/,/^},{/d" /usr/local/etc/xray/config.json
    systemctl restart xray > /dev/null 2>&1
    clear
    echo -e "\033[1;93m─────────────────────────────────────\033[0m"
...
echo -e "\033[1;93m└──────────────────────────────────────────┘\033[0m"
echo -e "\033[1;93m┌──────────────────────────────────────────┐\033[0m"
echo -e "\033[1;93m│  ${grenbo}1.${NC} \033[0;36mAdd Account Vless     "
echo -e "\033[1;93m│  ${grenbo}2.${NC} \033[0;36mDelete Account Vless     "
echo -e "\033[1;93m│  ${grenbo}3.${NC} \033[0;36mRenew Account Vless     "
echo -e "\033[1;93m│  ${grenbo}4.${NC} \033[0;36mCek User Vless     "
echo -e "\033[1;93m└──────────────────────────────────────────┘\033[0m"
echo -e ""
